...
#
# 重要: 如果回答被截断，stop_reason 会是 "length" 而不是 "stop"
...
def max_tokens_experiment():
    ...
    for max_t in [30, 100, 500]:
        ...
        print(f"  停止原因: {reason}")  # "stop"=正常结束, "length"=被截断
...
def system_prompt_comparison():
    ...
    print(f"  {r1.choices[0].message.content[:150]}...\n")
    ...
    r2 = client.chat.completions.create(
        ...
        max_tokens=200,
        messages=[
            {"role": "system", "content": "你是 Python 导师，用户是前端工程师。用 JavaScript 的概念类比解释，回答控制在 3 句话以内。"},
    ...
    r3 = client.chat.completions.create(
        ...
        max_tokens=200,
        messages=[
            {"role": "system", "content": "你是一个毒舌但专业的编程老师，回答时带点幽默和吐槽，但知识点要准确。"},
            {"role": "user", "content": question}
        ]
    )
...
def few_shot_demo():
    ...
    r2 = client.chat.completions.create(
        ...
        max_tokens=100,
        messages=[
            {"role": "system", "content": "你是一个 Git commit 信息生成器。根据用户描述的需求，生成规范的 commit message。"},
            # 示例 1
...
def hallucination_demo():
    ...
    response = client.chat.completions.create(
        ...
        max_tokens=100,
        messages=[
            {"role": "system", "content": "如果你不确定或不知道答案，请直接说"我不确定"，不要编造信息。"},
            {"role": "user", "content": "npm 包 super-fast-validator-2024 的作者是谁？"}
        ]
    )
    print(f"加约束后: {response.choices[0].message.content}\n")
